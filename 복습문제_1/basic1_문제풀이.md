좋습니다! 🎉 이제 앞서 제시한 **20문제(기초 10문제 + 중급 10문제)**에 대한 **자바 정답 코드**를 모두 작성해드릴게요.  
각 문제는 독립적으로 실행할 수 있도록 `main` 메서드 안에 작성된 예시 코드입니다.  
(실습 시에는 각 문제를 별도의 파일로 분리하거나, 하나의 파일에서 주석 처리 후 원하는 문제만 실행하면 됩니다.)
